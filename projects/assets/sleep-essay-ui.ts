...
(function() {
  function initSleepEssayUi() {
    ...
    const chapterLinks = Array.from(document.querySelectorAll<HTMLAnchorElement>('.chapter-link[href^="#"]'));
    ...
    function updateReadingState() {
      ...
      let active: { link: HTMLAnchorElement; section: HTMLElement } | null = null;
    ...
    function syncPressedState(group: HTMLElement) {
      group.querySelectorAll<HTMLButtonElement>('button').forEach(button => {
        button.setAttribute('aria-pressed', String(button.classList.contains('active')));
      });
    }

    function enhanceButtonGroup(id: string, observeChildren = false) {
