...
(function () {
    ...
    function updateHint() {
        ...
        if (hint)
            hint.textContent = `← → 翻页 · B ${window.__lowPowerMode ? '动态' : '静态'} · ESC 索引`;
    }
    window.__setLowPowerMode = function (on, opts = {}) {
        ...
        if (opts.persist !== false)
            localStorage.setItem(KEY, window.__lowPowerMode ? '1' : '0');
        ...
        if (window.__playSlide)
            window.__playSlide(window.__currentSlideIndex || 0);
    };
    document.body.classList.toggle('low-power', window.__lowPowerMode);
...
function startGrid() {
    if (document.body.classList.contains('canvas-mode') || window.__lowPowerMode || gridRAF)
        return;
    if (!gridCtrl)
        gridCtrl = bootGL('bg-grid', FS);
    if (!gridCtrl)
        return;
    gridT0 = Date.now();
    function loop() {
...
function go(n) {
    ...
    darkMode = isDark;
    if (window.__playSlide)
        setTimeout(() => window.__playSlide(idx), 450);
    lock = true;
...
addEventListener('keydown', e => {
    ...
    if (e.key === 'ArrowRight' || e.key === 'PageDown' || e.key === ' ' || e.key === 'ArrowDown') {
        if (window.__pipeAdvance && window.__pipeAdvance())
            return;
        go(idx + 1);
        return;
    }
...
addEventListener('touchend', e => {
    ...
    if (Math.abs(dx) > 50 && Math.abs(dx) > Math.abs(dy)) {
        if (dx < 0 && window.__pipeAdvance && window.__pipeAdvance())
            return;
        go(idx + (dx < 0 ? 1 : -1));
    }
}, { passive: true });
const params = new URLSearchParams(location.search);
if (params.get('mode') === 'dynamic')
    window.__setLowPowerMode(false, { persist: false });
if (params.get('mode') === 'static' || params.get('pdf') === '1')
...
(function () {
    ...
    function draw(c, t) {
        ...
        for (let r = 0; r < rows; r++) {
            for (let cc = 0; cc < cols; cc++) {
                const n = (Math.sin(cc * 0.18 + t) +
                    Math.sin(r * 0.24 - t * 0.7) +
                    Math.sin((cc + r) * 0.12 + t * 0.45) +
                    Math.sin(Math.hypot(cc - cols * 0.5, r - rows * 0.5) * 0.16 - t * 0.55)) / 4; // [-1, 1]
                const v = (n + 1) / 2; // [0, 1]
                if (v < 0.22)
                    continue;
    ...
    function tick(now) {
        if (!running || window.__lowPowerMode) {
            running = false;
            asciiRAF = 0;
            return;
        }
    ...
    function stop() {
        ...
        canvases.forEach(c => {
            if (c.__ctx)
                c.__ctx.clearRect(0, 0, c.__w || 0, c.__h || 0);
        });
    }
...
(function () {
    ...
    function parseFocuses(img) {
        ...
        return raw.split(';').map(function (part, idx) {
            ...
            var x = parseFloat(bits[1]);
            var y = parseFloat(bits[2]);
            var scale = parseFloat(bits[3]);
            return {
                label: label,
