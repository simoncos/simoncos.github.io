...
slides.forEach((s,i)=>{
  ...
  b.className='dot';b.dataset.i=String(i);b.setAttribute('aria-label','Page '+(i+1));
  b.onclick=()=>go(i);
  nav.appendChild(b);
});
...
(function(){
  const canvases = [...document.querySelectorAll<HTMLCanvasElement>('canvas.ascii-bg')];
...
(function(){
  ...
  function buildGallery(sec: Element){
    var imgs=Array.from(sec.querySelectorAll<HTMLImageElement>('img.zoomable'));
    var items=[];
