...
(function () {
    ...
    // ---- Board / Present --------------------------------------------------
    const track = board.querySelector('[data-track]');
    const modes = Array.from(board.querySelectorAll('[data-mode]'));
    const pos = board.querySelector('[data-slide-pos]');
    ...
    function syncPos() {
        if (pos)
            pos.textContent = `${slide + 1} / ${slides.length}`;
    }
    ...
    board.querySelectorAll('[data-slide]').forEach((button) => {
    ...
    document.addEventListener('keydown', (event) => {
        if (!present() || event.metaKey || event.ctrlKey || event.altKey)
            return;
    ...
    const YEARS = 10;
    const COLS = 366;
    const canvas = board.querySelector('[data-dots]');
    const countEl = board.querySelector('[data-dots-count]');
    const labelEl = board.querySelector('[data-dots-label]');
    ...
    function syncCount() {
        if (countEl)
            countEl.textContent = filled.toLocaleString('en-US');
    }
    if (canvas) {
        canvas.addEventListener('pointermove', (event) => {
            ...
            year = next;
            if (labelEl)
                labelEl.textContent = year === null ? labelEl.dataset.default || '' : String(2016 + year);
            drawDots();
        });
    ...
    // ---- Pipeline ---------------------------------------------------------
    const stageButtons = Array.from(board.querySelectorAll('[data-stage]'));
    const stageDescs = Array.from(board.querySelectorAll('[data-stage-desc]'));
    function setStage(i) {
    ...
    // ---- Privacy timeline -------------------------------------------------
    const priv = board.querySelector('[data-priv]');
    if (priv && !reduced) {
        const steps = Array.from(priv.querySelectorAll('li'));
        let lit = 0;
