...
(function () {
        ...
        scope.querySelectorAll('[data-i18n]').forEach((el) => {
            (el.getAttribute('data-i18n') || '').split(/\s+/).forEach((name) => {
                ...
                if (!el.hasAttribute(enKey))
                    el.setAttribute(enKey, el.getAttribute(name) || '');
    ...
    document.querySelectorAll('[data-lang-nav]').forEach((link) => {
        link.addEventListener('click', () => {
            ...
            if (target === 'zh' || target === 'en')
                store(localStorage, 'siteLanguage', target);
        });
    });
    // ---- Theme ----------------------------------------------------------
    ...
    if (themeQuery) {
        ...
        if (typeof themeQuery.addEventListener === 'function')
            themeQuery.addEventListener('change', follow);
    }
    // ---- Mobile menu ----------------------------------------------------
    ...
    function markExternal(scope) {
        scope.querySelectorAll('a[href^="http"]').forEach((link) => {
            if (link.hasAttribute('data-ext') || link.hasAttribute('data-noext'))
                return;
            let host = '';
            try {
            ...
            if (/[↗→]/.test(text) || link.querySelector('img, canvas'))
                link.setAttribute('data-noext', '');
            else
                link.setAttribute('data-ext', '');
        });
    }
    ...
    document.addEventListener('pointerover', (event) => {
        ...
        if (floatImg && floatImg.getAttribute('src') !== src)
            floatImg.src = src;
        el.classList.add('on');
    });
    document.addEventListener('pointerout', (event) => {
        ...
        if (next && floatHost.contains(next))
            return;
        floatHost = null;
