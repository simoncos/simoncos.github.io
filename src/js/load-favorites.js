...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const main = document.querySelector('.favorites-shell');
        const list = document.querySelector('[data-favorites-list]');
        const pager = document.querySelector('[data-favorites-pager]');
        const filterGroup = document.querySelector('.favorites-filter');
        if (!main || !list || !pager || !filterGroup) {
            return;
        }
        const rows = Array.from(list.querySelectorAll(':scope > .favorite-row'));
        const perPage = Number(main.dataset.favoritesPerPage) || 20;
        const unit = main.dataset.favoritesUnit || '';
        const filterButtons = Array.from(filterGroup.querySelectorAll('[data-favorites-filter]'));
        let state = readState();
        function readState() {
        ...
        function mainReview(row) {
            return row.querySelector('.favorite-notes > .favorite-review, .favorite-notes-main > .favorite-review');
        }
        function applyClamps(visibleRows) {
        ...
        function setToggleLabel(button, expanded, collapsedLabel) {
        ...
        filterButtons.forEach((button) => {
            button.addEventListener('click', () => {
                const filter = button.dataset.favoritesFilter;
        ...
        pager.addEventListener('click', (event) => {
            const link = event.target instanceof Element ? event.target.closest('a[data-favorites-page]') : null;
        ...
        list.addEventListener('click', (event) => {
            const button = event.target instanceof Element ? event.target.closest('.favorite-toggle') : null;
            if (!button) {
                return;
            }
            const row = button.closest('.favorite-row');
            ...
            if (button.classList.contains('favorite-series-toggle')) {
                ...
                const mainBlock = row?.querySelector('.favorite-notes-main');
                if (all) {
                    all.hidden = !expanded;
                }
                if (mainBlock) {
