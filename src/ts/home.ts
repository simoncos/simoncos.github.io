...
(function () {
    ...
    if (sel) {
        ...
        sel.addEventListener('animationend', (event) => {
            if (event.animationName !== 'om-bar' || shell?.reduced) return;
            const bar = (event.target as Element).closest('.sel-bar');
            if (bar && bar.classList.contains('is-on')) setActive(active + 1);
        });
        ...
        sel.addEventListener('focusout', (event) => {
            if (!sel.contains(event.relatedTarget as Node | null)) resume();
        });
