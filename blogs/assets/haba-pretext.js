...
function subtractObstacle(intervals, left, right) {
    const next = [];
    for (const interval of intervals) {
        ...
        if (left > interval.start)
            next.push({ start: interval.start, end: left });
        if (right < interval.end)
            next.push({ start: right, end: interval.end });
    }
    return next;
}
function freeIntervals(y, lineHeight, width, obstacles, padding) {
    let intervals = [{ start: padding, end: width - padding }];
    for (const obstacle of obstacles) {
        const intersects = obstacle.bottom + OBSTACLE_GAP > y
            && obstacle.top - OBSTACLE_GAP < y + lineHeight;
        if (!intersects)
            continue;
        intervals = subtractObstacle(intervals, Math.max(padding, obstacle.left - OBSTACLE_GAP), Math.min(width - padding, obstacle.right + OBSTACLE_GAP));
    }
    return intervals.filter(interval => interval.end - interval.start >= MIN_INTERVAL);
}
function boot() {
    const sceneQuery = document.querySelector('.haba-flow-scene');
    if (!(sceneQuery instanceof HTMLElement))
        return;
    const scene = sceneQuery;
    const stageQuery = scene.querySelector('[data-haba-flow-stage]');
    const canvasQuery = scene.querySelector('[data-haba-flow-canvas]');
    const staticTextQuery = scene.querySelector('[data-haba-flow-static]');
    if (!(stageQuery instanceof HTMLElement)
        || !(canvasQuery instanceof HTMLCanvasElement)
        || !(staticTextQuery instanceof HTMLElement))
        return;
    const stage = stageQuery;
    const canvas = canvasQuery;
    const staticText = staticTextQuery;
    ...
    function getObstacles() {
        return Array.from(stage.querySelectorAll('[data-haba-obstacle]'))
            .map(element => ({
    ...
    function render() {
        scheduledFrame = null;
        if (!stage.classList.contains('is-enhanced') || context === null)
            return;
        ...
        while (y + lineHeight <= height - bottomPadding) {
            ...
            if (range === null || sameCursor(cursor, range.end))
                break;
            const line = materializeLineRange(prepared, range);
            if (line.text.trim())
                context.fillText(line.text, interval.start, y);
            cursor = range.end;
        ...
        if (remaining !== null) {
            const expandedHeight = Math.min(MAX_STAGE_HEIGHT, stage.offsetHeight + STAGE_GROWTH_STEP);
        ...
        const lowestObstacle = obstacles.reduce((lowest, obstacle) => Math.max(lowest, obstacle.bottom), 0);
        const fittedHeight = Math.max(MIN_STAGE_HEIGHT, Math.ceil(Math.max(lastDrawnBottom, lowestObstacle)
            + bottomPadding
            + borderBoxDelta));
